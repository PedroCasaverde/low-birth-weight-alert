...
class EndesProcessor:
    ...
    def procesar_modulo_1(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        """
        print(
            f"Iniciando el procesamiento del Módulo 1 (RECH0, RECH1, RECH4) para el año {anio}..."
        )
        ...
        rech0_vars = {
            "HHID": "id_hogar",
            "HV001": "id_conglomerado",
            "HV002": "id_vivienda",
            "HV005": "factor_ponderacion_hogar",
            "HV024": "region",
            "HV025": "area_residencia",
            "HV040": "altitud_metros",
            "UBIGEO": "ubigeo",
            "LATITUDY": "latitud",
            "LONGITUDX": "longitud",
        }
        rech1_vars = {
            "HHID": "id_hogar",
            "HVIDX": "id_miembro_hogar",
            "HV101": "parentesco_jefe_hogar",
            "HV104": "sexo",
            "HV105": "edad_anios",
            "HV109": "nivel_educativo",
        }
        ...
        try:
            df_miembros = pd.read_csv(
                paths["rech1"],
                usecols=rech1_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
        ...
        try:
            ...
            print(
                f"Advertencia: No se pudo procesar RECH4. Razón: {e}. Se continuará sin datos de seguros."
            )
        ...
        df_consolidado = pd.merge(left=df_miembros, right=df_hogar, on="id_hogar", how="left")
        ...
        print(
            f"Proceso del Módulo 1 completado. DataFrame con {df_consolidado.shape[0]} filas y {df_consolidado.shape[1]} columnas."
        )
    ...
    def procesar_modulo_2(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        try:
            ...
            cols_to_load = [col for col in rech23_vars if col in available_cols]

            df_hogar_caracteristicas = pd.read_csv(
                file_path, usecols=cols_to_load, dtype=id_dtypes, low_memory=False
            )
            ...
            print(
                f"Proceso del Módulo 2 completado. DataFrame con {df_hogar_caracteristicas.shape[0]} filas y {df_hogar_caracteristicas.shape[1]} columnas."
            )
    ...
    def procesar_modulo_3(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        rec0111_vars = {
            ...
            "HHID": "id_hogar",
            "V012": "edad_mujer",
            "V102": "area_residencia_mujer",
            "V106": "nivel_educativo_mujer",
            "V130": "religion_mujer",
            "V157": "frecuencia_lee_periodico",
            "V158": "frecuencia_escucha_radio",
            "V159": "frecuencia_ve_tv",
            "V190": "quintil_riqueza_mujer",
        ...
        try:
            df_rec0111 = pd.read_csv(
                paths["rec0111"],
                usecols=rec0111_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
        ...
        try:
            ...
            cols_to_load_original = [
                col for col in rec91_vars if col.replace(" ", "_") in temp_df.columns
            ]

            df_rec91 = pd.read_csv(
                paths["rec91"],
                usecols=cols_to_load_original,
                dtype=id_dtypes,
                low_memory=False,
            )
        ...
        print(
            f"Proceso del Módulo 3 completado. DataFrame con {df_consolidado.shape[0]} filas y {df_consolidado.shape[1]} columnas."
        )
        return df_consolidado
    ...
    def procesar_modulo_4(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        # Cargar RE223132
        try:
            df_repro_summary = pd.read_csv(
                paths["re223132"],
                usecols=re223132_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
            ...
            print(
                f"Advertencia: No se pudo procesar RE223132. Razón: {e}. Se continuará sin estos datos."
            )
            df_repro_summary = pd.DataFrame()

        # Cargar REC21 (Listado de Nacimientos)
        try:
            df_birth_roster = pd.read_csv(
                paths["rec21"],
                usecols=rec21_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
            ...
            print(
                f"Error cargando Módulo REC21: {e}. Este módulo es esencial, el proceso no puede continuar."
            )
            return pd.DataFrame()
        ...
        print(
            f"Proceso del Módulo 4 completado. DataFrame con {df_consolidado.shape[0]} filas y {df_consolidado.shape[1]} columnas."
        )
        return df_consolidado
    ...
    def procesar_modulo_5(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        rec41_vars = {
            "CASEID": "id_cuestionario_mujer",
            "MIDX": "id_nacimiento",
            "M14": "controles_prenatales_num",
            "M15": "lugar_parto",
            "M17": "parto_fue_cesarea",
            "M19": "peso_bebe_nacimiento_kg",
            "M45": "consumio_suplemento_hierro_embarazo",
            "M70": "bebe_tuvo_control_medico_1er_mes",
        }
        rec94_vars = {
            "CASEID": "id_cuestionario_mujer",
            "IDX94": "id_nacimiento",
            "S413": "madre_afiliada_sis_embarazo",
            "S426GB": "complicacion_parto_sangrado_excesivo",
            "S427DA": "complicacion_postparto_sangrado_intenso",
            "Q1422A A": "se_hizo_prueba_anemia_embarazo",
            "Q1422A B": "dx_anemia_embarazo",
            "Q1422A C": "recibio_tratamiento_hierro_embarazo",
            "Q1422A D": "cumplio_tratamiento_hierro_embarazo",
        }
        ...
        # Procesar REC41
        try:
            df_rec41 = pd.read_csv(
                paths["rec41"],
                usecols=rec41_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
        ...
        try:
            ...
            cols_to_load_original = [
                col for col in rec94_vars if col.replace(" ", "_") in temp_df.columns
            ]

            df_rec94_raw = pd.read_csv(
                paths["rec94"],
                usecols=cols_to_load_original,
                dtype=id_dtypes,
                low_memory=False,
            ...
            print(
                f"Advertencia: No se pudo procesar REC94. Razón: {e}. Se continuará sin estos datos."
            )
        ...
        if not df_rec94.empty:
            df_final = pd.merge(
                df_rec41,
                df_rec94,
                on=["id_cuestionario_mujer", "id_nacimiento"],
                how="left",
            )
        else:
            df_final = df_rec41
        ...
        print(
            f"Módulo 5 reprocesado. DataFrame con {df_final.shape[0]} filas y {df_final.shape[1]} columnas."
        )
        return df_final
    ...
    def procesar_modulo_6(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        rec43_vars = {
            "CASEID": "id_cuestionario_mujer",
            "HIDX": "id_nacimiento",
            "H1": "tiene_carnet_salud",
            "H9": "vacuna_sarampion",
            "H11": "tuvo_diarrea_ult_2_semanas",
            "H22": "tuvo_fiebre_ult_2_semanas",
            "H31": "tuvo_tos_ult_2_semanas",
            "H42": "tomo_suplemento_hierro",
        }
        rec95_vars = {
            "CASEID": "id_cuestionario_mujer",
            "IDX95": "id_nacimiento",
            "S466": "tuvo_control_cred",
            "S465EA": "consumio_hierro_jarabe_ult_7_dias",
            "S465EB": "consumio_micronutrientes_ult_7_dias",
        ...
        rec42_vars = {
            ...
            "V438": "madre_talla_cm",
            "V445": "madre_imc",
            "V456": "madre_hemoglobina_ajustada",
            "V457": "madre_nivel_anemia",
            "V414H": "dieta_nino_carnes_ayer",
            "V414G": "dieta_nino_huevos_ayer",
        }
        ...
        for name, path, var_map in child_modules:
            try:
                ...
                cols_to_load_original = [
                    ...
                    if col.replace(" ", "_").replace(".", "") in temp_df.columns
                ]
                ...
                df = df.rename(
                    columns={k.replace(" ", "_").replace(".", ""): v for k, v in var_map.items()}
                )
        ...
        df_nino_consolidado = reduce(
            lambda left, right: pd.merge(
                left, right, on=["id_cuestionario_mujer", "id_nacimiento"], how="outer"
            ),
            dfs_nino,
        )
        ...
        try:
            df_mujer = pd.read_csv(
                paths["rec42"],
                usecols=rec42_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
        ...
        if not df_mujer.empty:
            df_final = pd.merge(
                df_nino_consolidado, df_mujer, on=["id_cuestionario_mujer"], how="left"
            )
        else:
            df_final = df_nino_consolidado
        df_final["id_hogar"] = df_final["id_cuestionario_mujer"].str[:9]
        print(
            f"Módulo 6 reprocesado. DataFrame con {df_final.shape[0]} filas y {df_final.shape[1]} columnas."
        )
        return df_final
    ...
    def procesar_modulo_7(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        try:
            ...
            print(
                f"Módulo 7 procesado. DataFrame con {df_modulo_7.shape[0]} filas y {df_modulo_7.shape[1]} columnas."
            )
            return df_modulo_7
    ...
    def procesar_modulo_8(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        """
        print(
            f"Iniciando el procesamiento final del Módulo 8 (RE758081, REC82) para el año {anio}..."
        )
        ...
        # Procesar RE758081
        try:
            df_re758081 = pd.read_csv(
                paths["re758081"],
                usecols=re758081_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
            ...
            print(
                f"Advertencia: No se pudo procesar RE758081. Razón: {e}. Se continuará sin estos datos."
            )
            df_re758081 = pd.DataFrame()

        # Procesar REC82
        try:
            df_rec82 = pd.read_csv(
                paths["rec82"],
                usecols=rec82_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
            ...
            print(
                f"Advertencia: No se pudo procesar REC82. Razón: {e}. Se continuará sin estos datos."
            )
        ...
        print(
            f"Módulo 8 procesado. DataFrame con {df_final.shape[0]} filas y {df_final.shape[1]} columnas."
        )
        return df_final
    ...
    def procesar_modulo_9(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        # Procesar REC84DV
        try:
            df_rec84dv = pd.read_csv(
                paths["rec84dv"],
                usecols=rec84dv_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
            ...
            print(
                f"Advertencia: No se pudo procesar REC84DV. Razón: {e}. Se continuará sin estos datos."
            )
        ...
        try:
            df_rec83 = pd.read_csv(
                paths["rec83"], usecols=["CASEID"], dtype=id_dtypes, low_memory=False
            )
        ...
        if not df_rec84dv.empty and not df_rec83_indicator.empty:
            ...
            df_final["reporto_mortalidad_hermanas"] = df_final[
                "reporto_mortalidad_hermanas"
            ].fillna(0)
        ...
        print(
            f"Módulo 9 procesado. DataFrame con {df_final.shape[0]} filas y {df_final.shape[1]} columnas."
        )
        return df_final
    ...
    def procesar_modulo_10(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        """
        print(
            f"Iniciando el procesamiento final del Módulo 10 (RECH5, RECH6) para el año {anio}..."
        )
        ...
        rech5_vars = {
            "HHID": "id_hogar",
            "HAD": "id_miembro_hogar",
            "HA1": "edad_mujer_medicion",
            "HA2": "peso_kg_mujer",
            "HA3": "talla_cm_mujer",
            "HA40": "imc_mujer",
            "HA53": "hemoglobina_mujer",
            "HA57": "anemia_nivel_mujer",
        }

        # Mapeo para RECH6 (Niños)
        rech6_vars = {
            "HHID": "id_hogar",
            "HC0": "id_miembro_hogar",
            "HC1": "edad_meses_nino",
            "HC2": "peso_kg_nino",
            "HC3": "talla_cm_nino",
            "HC53": "hemoglobina_nino",
            "HC57": "anemia_nivel_nino",
        }
        ...
        print(
            f"Módulo 10 procesado. DataFrame con {df_final.shape[0]} filas y {df_final.shape[1]} columnas."
        )
        return df_final
    ...
    def procesar_modulo_11(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        try:
            df_modulo_11 = pd.read_csv(
                file_path, usecols=rec93_vars.keys(), dtype=id_dtypes, low_memory=False
            )
            ...
            if "disciplina_madre_explica_conducta" in df_modulo_11.columns:
                df_modulo_11["disciplina_madre_explica_conducta"] = (
                    df_modulo_11["disciplina_madre_explica_conducta"]
                    .str.contains("K", na=False)
                    .astype(int)
                )

            if "disciplina_padre_castigo_fisico" in df_modulo_11.columns:
                df_modulo_11["disciplina_padre_castigo_fisico"] = (
                    df_modulo_11["disciplina_padre_castigo_fisico"]
                    .str.contains("E", na=False)
                    .astype(int)
                )
            ...
            print(
                f"Módulo 11 procesado. DataFrame con {df_modulo_11.shape[0]} filas y {df_modulo_11.shape[1]} columnas."
            )
            return df_modulo_11
    ...
    def procesar_modulo_12(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        """
        print(
            f"Iniciando el procesamiento del Módulo 12 (CSALUD01, CSALUD08) para el año {anio}..."
        )
        ...
        try:
            df_adultos = pd.read_csv(
                paths["csalud01"],
                usecols=csalud01_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
        ...
        try:
            df_ninos = pd.read_csv(
                paths["csalud08"],
                usecols=csalud08_vars.keys(),
                dtype=id_dtypes,
                low_memory=False,
            )
        ...
        print(
            f"Módulo 12 procesado. DataFrame con {df_final.shape[0]} filas y {df_final.shape[1]} columnas."
        )
        return df_final
    ...
    def procesar_modulo_13(self, data_path: str, anio: str) -> pd.DataFrame:
        ...
        """
        print(
            f"Iniciando el procesamiento del Módulo 13 (Programas Sociales x Hogar) para el año {anio}..."
        )
        ...
        try:
            ...
            cols_to_load = [col for col in ps_hogar_vars if col in available_cols]
            ...
            for col in df_programas.columns:
                if col != "id_hogar":
                    ...
                    df_programas.loc[:, col] = df_programas[col].apply(
                        lambda x: 1 if x == 1 or x == "1" else 0
                    )

            print(
                f"Módulo 13 procesado. DataFrame con {df_programas.shape[0]} filas y {df_programas.shape[1]} columnas."
            )
            return df_programas

        except FileNotFoundError:
            print(
                f"Error: No se encontró el archivo {file_path}. Por favor, verifica la ruta y el nombre del archivo."
            )
            return pd.DataFrame()
        except Exception as e:
